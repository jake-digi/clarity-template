    ...
    <div className="flex gap-4 overflow-x-auto pb-4" style={{ minHeight: 400 }}>
      {grouped.map((col) => (
        <div key={col.key} className="flex-shrink-0 w-[280px] flex flex-col">
          ...
          {/* Cards */}
          <div className="flex-1 bg-muted/30 border border-t-0 border-border rounded-b-md p-2 space-y-2 overflow-y-auto max-h-[calc(100vh-320px)]">
            {col.items.length === 0 && (
            ...
            {col.items.map((c) => (
              <Card
                key={c.id}
                className="p-3 cursor-pointer hover:shadow-md transition-shadow border-border"
