...
import { ArrowUpDown, X, Settings2, ShieldCheck, CheckCircle2, AlertCircle, Clock, Search, FileWarning } from "lucide-react";
...
export type QualityLot = {
    id: string;
    category: "Welding" | "Dimensional" | "Material" | "Electrical" | "Mechanical";
    title: string;
    responsibleParty: string;
    project: string;
    phase: string;
    severity: "CRITICAL" | "MAJOR" | "MINOR" | "NONE";
    status: "PASS" | "FAIL" | "PENDING" | "IN_PROGRESS" | "OPEN" | "CLOSED";
    dateRaised: string;
    assignedTo: string;
    ncr: string | null; // NCR Number if exists
};

// --- Mock Data ---
const initialLots: QualityLot[] = [
    {
        id: "QC-001",
        category: "Welding",
        title: "Main Chassis Joint Integrity",
        responsibleParty: "Production Team A",
    ...
    },
    {
        ...
        title: "Robot Base Alignment",
        responsibleParty: "Supplier: MechCorp",
        project: "Project Phoenix",
        phase: "Assembly",
        severity: "MAJOR",
        status: "FAIL",
    ...
    },
    {
        id: "QC-003",
        category: "Electrical",
        title: "Safety Loop Continuity",
        responsibleParty: "Elec-Team",
        project: "Skyline Facility",
        phase: "Wiring",
        severity: "CRITICAL",
        status: "OPEN",
    ...
    },
    {
        id: "QC-004",
        category: "Mechanical",
        title: "Pneumatic Fitting Torque",
        responsibleParty: "Production Team B",
        project: "Skyline Facility",
        phase: "Assembly",
        severity: "MINOR",
        status: "IN_PROGRESS",
        dateRaised: "2025-12-05",
        assignedTo: "John Smith",
        ncr: null
    },
    {
        id: "QC-005",
        category: "Material",
        title: "Bracket Material Certs",
        responsibleParty: "Purchasing",
        project: "Project Phoenix",
        phase: "Procurement",
        severity: "NONE",
        status: "PASS",
        dateRaised: "2025-12-07",
        assignedTo: "Alice Wong",
        ncr: null
    },
];
...
const getStatusIcon = (status: string) => {
    switch (status) {
        case "PASS": case "CLOSED": return <CheckCircle2 className="w-3 h-3 mr-1" />;
        case "FAIL": case "OPEN": return <AlertCircle className="w-3 h-3 mr-1 text-white" />;
...
export function QualityTable() {
    const [data] = useState<QualityLot[]>(initialLots);
    ...
    const columns: ColumnDef<QualityLot>[] = [
        {
            id: "select",
            size: 40,
        ...
        },
        {
            ...
            cell: ({ row }) => <div className="text-[10px] font-bold uppercase tracking-tight text-muted-foreground/70">{row.getValue("phase")}</div>,
        },
        {
        ...
        },
        {
            ...
            cell: ({ row }) => {
                const ncr = row.getValue("ncr") as string;
                return ncr ? (
                    <div className="flex items-center text-[11px] font-mono text-red-500 font-bold">
    ...
    return (
        <div className="w-full h-full flex flex-col min-h-0">
            ...
            <div className="flex-none flex items-center h-10 bg-background border-b border-border shadow-sm overflow-hidden">
                ...
                <Select value={(table.getColumn("severity")?.getFilterValue() as string) ?? "all"} onValueChange={(v) => table.getColumn("severity")?.setFilterValue(v === "all" ? undefined : v)}>
                    <SelectTrigger className="w-[110px] h-full text-xs bg-transparent border-none rounded-none border-r border-border hover:bg-muted/50 px-4 focus:ring-0 shadow-none">
                        <SelectValue placeholder="Severity">
                            {table.getColumn("severity")?.getFilterValue() ? (table.getColumn("severity")?.getFilterValue() as string) : "Severity"}
                        </SelectValue>
                    </SelectTrigger>
                    <SelectContent><SelectItem value="all">All Severities</SelectItem>{severities.map((s) => (<SelectItem key={s} value={s} className="text-xs">{s}</SelectItem>))}</SelectContent>
                </Select>
            ...
            <DataTable table={table} columns={columns} selectionLabel="Lot(s)" />
        </div>
    );
}
