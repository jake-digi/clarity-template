            ...
            <div>
                ...
                >
                    ...
                    <div className="flex-shrink-0">
                        {Icon && <Icon className={cn("w-4 h-4", isExpanded ? "text-primary" : "text-muted-foreground")} />}
                    </div>
                    <span className={cn(
                        "truncate",
...
const AdminSidebar = ({
    ...
    isCollapsed: boolean;
    onSelectAdminAction: (action: { id: string, name: string }) => void;
    onNewAdminAction?: () => void;
}) => {
    ...
    const [sections, setSections] = useState<CollectionItem[]>([
        {
            ...
            expanded: true,
            children: [
                { id: 'users', name: 'User Management', type: 'item', icon: Users },
                { id: 'roles', name: 'Roles & Permissions', type: 'item', icon: Key },
                { id: 'api-keys', name: 'Service Accounts', type: 'item', icon: Cloud },
            ],
        },
        {
            id: 'system',
            ...
            expanded: true,
            children: [
                { id: 'audit-logs', name: 'Security Audit', type: 'item', icon: Activity },
                { id: 'backups', name: 'System Backups', type: 'item', icon: Database },
            ],
        },
    ]);
    ...
    return (
        <SidebarContainer isCollapsed={isCollapsed}>
            ...
            <SidebarFooter className="border-t border-sidebar-border bg-sidebar/20 p-2">
                <SidebarActionButton label="Global Config" variant="ghost" icon={Settings2} className="h-9 justify-start px-2 font-semibold text-[12px] border-none" />
            </SidebarFooter>
        </SidebarContainer>
    );
};
